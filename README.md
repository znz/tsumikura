...
> **現在の状態: 実装の途中。** [実装計画](docs/plan/implementation-plan.md) の Phase 7 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理、Dokku デプロイの準備、マスタと品目、在庫台帳と購入)。品目を登録して購入や初期在庫を記録でき、ロットごとの残数・期限・単価が見られる。使用の記録、棚卸、予測の表示、買い物リストはまだ無い。ローカルの `bin/ci` は緑で、GitHub Actions と Dokku への初回デプロイ ([手順書](docs/ops/first-deploy.md)) は未実施。
