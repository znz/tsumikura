...
> **現在の状態: 実装の終盤。** [実装計画](docs/plan/implementation-plan.md) の Phase 11 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理、Dokku デプロイの準備、マスタと品目、在庫台帳と購入、使用記録と用途、棚卸と廃棄、予測の結線とダッシュボード、買い物リスト)。購入・使用・棚卸・廃棄を記録でき、ダッシュボードに要購入と期限のアラートが出て、買い物リストでチェックした品目をまとめて購入として記録できる。通知 (PWA / Web Push) とパスキーはまだ無い。ローカルの `bin/ci` は緑で、GitHub Actions と Dokku への初回デプロイ ([手順書](docs/ops/first-deploy.md)) は未実施。
