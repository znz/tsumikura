...
> **現在の状態: 実装の初期段階。** [実装計画](docs/plan/implementation-plan.md) の Phase 4 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理)。ログインして空のダッシュボードが見られ、管理者が家族のアカウントを追加できる。品目や在庫の機能はまだ無い。ローカルの `bin/ci` は緑で、GitHub Actions は初回 push で確認する。
