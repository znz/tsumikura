...
> **現在の状態: 実装の終盤。** [実装計画](docs/plan/implementation-plan.md) の Phase 12 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理、Dokku デプロイの準備、マスタと品目、在庫台帳と購入、使用記録と用途、棚卸と廃棄、予測の結線とダッシュボード、買い物リスト、PWA と Web Push)。残りはパスキー (Phase 13) だけ。ローカルの `bin/ci` は緑。GitHub Actions、Dokku への初回デプロイ ([手順書](docs/ops/first-deploy.md))、通知の実機確認 ([手順](docs/spec/04-notifications.md))、アイコン画像の差し替えは未実施 ([未決事項](docs/plan/open-questions.md))。
