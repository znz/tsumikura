...
> **現在の状態: 実装の途中。** [実装計画](docs/plan/implementation-plan.md) の Phase 6 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理、Dokku デプロイの準備、マスタと品目)。ログインして品目とカテゴリ・保管場所・店舗を登録でき、管理者が家族のアカウントを追加できる。在庫の増減 (購入・使用・棚卸) と予測の表示はまだ無い。ローカルの `bin/ci` は緑で、GitHub Actions と Dokku への初回デプロイ ([手順書](docs/ops/first-deploy.md)) は未実施。
