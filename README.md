...
> **現在の状態: 実装の途中。** [実装計画](docs/plan/implementation-plan.md) の Phase 8 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理、Dokku デプロイの準備、マスタと品目、在庫台帳と購入、使用記録と用途)。品目を登録して購入を記録し、一覧の「使った」を 1 タップして在庫を減らせる (取り消しつき)。用途ごとの交換履歴も見られる。棚卸と廃棄、予測の表示、買い物リスト、通知、パスキーはまだ無い。ローカルの `bin/ci` は緑で、GitHub Actions と Dokku への初回デプロイ ([手順書](docs/ops/first-deploy.md)) は未実施。
