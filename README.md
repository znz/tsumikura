...
> **現在の状態: 実装の途中。** [実装計画](docs/plan/implementation-plan.md) の Phase 10 まで完了 (基盤整備、テスト基盤、予測ロジック、認証・ユーザー管理、Dokku デプロイの準備、マスタと品目、在庫台帳と購入、使用記録と用途、棚卸と廃棄、予測の結線とダッシュボード)。購入・使用・棚卸・廃棄を記録でき、ダッシュボードに「購入推奨 / そろそろ / 期限切れ / 期限間近」が出て、品目ごとに在庫切れ予測日と消費ペースが見られる。買い物リスト、通知 (PWA / Web Push)、パスキーはまだ無い。ローカルの `bin/ci` は緑で、GitHub Actions と Dokku への初回デプロイ ([手順書](docs/ops/first-deploy.md)) は未実施。
