...
| 3 | 品目詳細 | `/items/:id` | 在庫の大表示、「使った」/「使用を記録」/「購入を記録」、予測 (在庫切れ予測日・ペース「約 2.3 個/月」・判定理由)、最低在庫、ロット一覧 (期限順・残数・期限バッジ・購入日・店舗・単価。残 0 は畳む。平均単価。調整ロットには編集リンクを出さない)、用途一覧 (最終交換日・交換周期の中央値。用途マスタへの導線)、最近の記録 (使用と購入の時系列 10 件・各行から編集 / 削除)、操作ボタン群 (編集 / アーカイブ。**物理削除は提供しない**。アーカイブ済みならその旨と解除ボタン) |
...
- **ワンタップ使用の応答**は 2 系統を同じ部分テンプレート (`layouts/_toast`) で作る。
  ...
  - HTML (JS 無し): 押した画面に戻し (`redirect_back_or_to`。外部サイトの Referer は無視して品目詳細へ)、
    `flash[:toast]` と `flash[:undo_usage_record_id]` からレイアウトが同じトーストを描く。
