...
- **最小 12 文字** (`User::MINIMUM_PASSWORD_LENGTH`)。最大は `has_secure_password` の 72 バイト。
- 8 文字で始めたが、公開インターネットに出したので **2026-09-21 に 12 文字へ引き上げた** ([未決事項](../plan/open-questions.md) の「決定済み」、旧 #10)。8 文字は NIST SP 800-63B の**下限**であって推奨値ではなく、常用手段がパスキーに移った今は手入力の機会が少ない。
- 複雑さの要件 (記号必須など) は課さない。長さだけを見る。
- **検証はパスワードを設定・変更するときだけ走る** (`validates :password, length: ..., allow_nil: true`)。したがって:
  - 引き上げ前に作られた短いパスワードのユーザーは、**そのままログインでき、パスワード以外の属性も更新できる**。再設定を強制しない (強制する手段も持たない)。
  - そのユーザーが次にパスワードを変えるときは 12 文字以上を求められる。
  - この 2 つは `spec/models/user_spec.rb` と `spec/requests/sessions_spec.rb` で固定してある。
- 管理者が発行する生成パスワード (`User.generate_password`、4 文字 × 4 群 + 区切り = 19 文字) と `tsumikura:create_admin` の自動生成は影響を受けない。`ADMIN_PASSWORD=` で明示指定したパスワードが短いときに abort する挙動は、そのまま 12 文字基準になる (メッセージはモデルの検証エラーから出る)。
- 画面のヒント (「12 文字以上」) は `User::MINIMUM_PASSWORD_LENGTH` から描くので、定数を変えれば追従する。
...
| 端末 | 状況 |
|---|---|
| **PC (Windows Hello / Touch ID など)** | **確認済み**。登録でき、「パスキーでログイン」ボタンからも、メール欄の入力候補 (conditional UI) からもログインできた |
| PC (USB セキュリティキー) | 未確認 (手持ちが無い場合は省略してよい。PIN 要求の挙動だけ下に残す) |
| Android (Google パスワードマネージャー) | **未確認**。通知は Android で確認済みなので、同じ端末で試せる |
| iPhone / iPad (Face ID / Touch ID) | **未確認**。Safari のユーザー操作要件 (下の 3.) に引っかかるとしたらここ |
